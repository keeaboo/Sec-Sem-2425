...
int main(){
    ...
    printf("\nDwarf Problem (Array Name acting as Pointer Version)\n");
    ...
    for(int i = 0; i < 5; i++) {
        ...
        sumFactors(*(arr + i), pSum);

        numberHalf(*(arr + i), pSum, pHalf);

        if (isItDwarf(pSum, pHalf)) 
            printf("%d is a DWARF", *(arr + i));
        else 
            printf("%d is not a DWARF", *(arr + i));
